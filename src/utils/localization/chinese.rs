...
pub fn get_text(key: TextKey) -> &'static str {
    match key {
        ...
        TextKey::ErrorDetails => "错误详情",
         ...
         TextKey::NextStepsList => "1. 重启两台电脑\n2. 按照指南中的后续步骤操作\n   - 在主机上安装固件驱动程序\n   - 将电缆更换到 DATA 端口\n   - 使用提供的软件和激活码进行激活\n   - DNA 锁定的固件版本无需激活",
         TextKey::Exit => "退出",
         TextKey::MainMenu => "主菜单",
         TextKey::TryAgainButton => "重试",

        // Detailed Result Messages
        TextKey::DnaReadUnexpectedMsg => "操作完成，但无法确认 DNA 值。\n这可能表明 DNA 提取过程存在问题。\n请检查日志输出以获取详细信息。",
        ...
        TextKey::FlashingFailedConnectionMsg => "检测到的正常扇区写入不足： {} / {} 扇区。\n\n这表明存在硬件连接问题。设备虽然可访问，但数据未正确传输。\n\n尝试：\n1. 使用不同的 USB 端口\n2. 检查线缆连接\n3. 确保设备供电正常\n4. 尝试更换 USB 线缆",
        TextKey::FlashingResultUnknownMsg => "烧录过程已完成，但在日志中未找到扇区写入信息。\n\n1. 确认您选择了正确的板卡类型\n2. 确认已安装正确的 USB 驱动程序并连接到 JTAG 端口。\n3. 尝试更换 USB 线缆和/或端口\n4. 确保设备已正确安装在 PCIE 插槽中。",
